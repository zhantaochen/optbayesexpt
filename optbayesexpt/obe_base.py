...
class OptBayesExpt(ParticlePDF):
    ...
    **Attributes:**
    ...
    def set_selection_method(self, selection_method):
        ...
        elif selection_method == 'unique_optimal':
            self.get_setting = self.unique_opt_setting
        else:
    ...
    def unique_opt_setting(self, measured_settings):
        """Find the setting with maximum utility and outside measured settings
        ...
        Returns:
            A settings tuple.
        """
        if len(measured_settings) > 0:
            mask_valid_settings = np.min(
                np.round(np.abs(np.asarray(measured_settings) - self.allsettings), 3), axis=0
                ).astype('bool')
        else:
            mask_valid_settings = np.ones(self.allsettings.shape[1]).astype('bool')
        ...
        utility[~mask_valid_settings] = utility.min() - 1.0
